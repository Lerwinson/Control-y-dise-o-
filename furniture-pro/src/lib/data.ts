...
import type { Material, Part, Project, Template } from './types';
...
export function makePart(p: Partial<Part> = {}): Part {
  return {
    id: p.id || uid('prt'),
    code: p.code || 'P-000',
    name: p.name || 'Pieza',
...
export function seedProjects(): Project[] {
  const now = Date.now();
  return [
    { id: uid('prj'), name: 'Cama Queen — Cliente A', type: 'bed', status: 'active', stage: 1, createdAt: now - 86400000 * 9, updatedAt: now - 3600000 * 5, currency: 'USD', margin: 35, parts: bedFrameParts(), shapes2d: [], boxes3d: [] },
    { id: uid('prj'), name: 'Sofá 3 plazas — Showroom', type: 'sofa', status: 'active', stage: 3, createdAt: now - 86400000 * 5, updatedAt: now - 3600000 * 30, currency: 'USD', margin: 42, parts: sofaParts(), shapes2d: [], boxes3d: [] },
    { id: uid('prj'), name: 'Mesa comedor — Lote 12', type: 'furniture', status: 'active', stage: 5, createdAt: now - 86400000 * 2, updatedAt: now - 3600000 * 2, currency: 'USD', margin: 28, parts: tableParts(), shapes2d: [], boxes3d: [] },
  ];
}
